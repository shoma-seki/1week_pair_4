...
public class RunwaySpotlights : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        {
            Debug.LogWarning("MusicManager was not found, so runway spotlights cannot sync to the beat.", this);
            return;
        }
    ...
    {
        activePair = Mathf.Abs(beatIndex) % pairCount;
    ...
    {
        activePair = -1;
        SetAllLights(peakIntensity, true);
    }
    ...
    private void SetAllLights(float intensity, bool enabled)
    {
        for (int i = 0; i < pairCount; i++)
        {
            SetLight(leftLights[i], intensity, enabled);
            SetLight(rightLights[i], intensity, enabled);
            UpdateBeam(leftLights[i], leftBeams[i]);
            UpdateBeam(rightLights[i], rightBeams[i]);
        }
    }

    private static void SetLight(Light spotlight, float intensity, bool enabled)
    {
        if (spotlight == null)
        {
            return;
